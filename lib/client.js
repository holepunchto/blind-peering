...
const { AddCoreEncoding, DeleteCoreEncoding, AddCoresEncoding } = require('blind-peer-encodings')
...
class BlindPeerClient extends ReadyResource {
  constructor(remotePublicKey, dht, opts = {}) {
    ...
    this.backoffValues = opts.backoffValues || [
      1000, 1000, 1000, 2000, 2000, 3000, 3000, 5000, 5000, 15000, 30000, 60000
    ]
  ...
  async _connect() {
    ...
    while (!this.closing && !this.suspended && !this.dht.destroyed) {
      const socket = this.dht.connect(this.remotePublicKey, {
  ...
  async deleteCore(key) {
    ...
    for (let i = 0; i < this.backoffValues.length * 2; i++) {
      ...
      try {
        return await this.rpc.request('delete-core', req, DeleteCoreEncoding)
      } catch (err) {
        error = err
      }
    }
  ...
  async addCore(id, opts = {}) {
    ...
    for (let i = 0; i < this.backoffValues.length * 2; i++) {
      ...
      try {
        return await this.rpc.request('add-core', req, AddCoreEncoding)
      } catch (err) {
        error = err
      }
    }
  ...
  async addAutobaseCores(cores, referrer, { priority = 0 } = {}) {
    const items = cores.map((c) => {
      return { key: c.key, length: c.length }
    })
    ...
    // TODO: refactor this to deduplicate this connection code
    for (let i = 0; i < this.backoffValues.length * 2; i++) {
      while (!this.closing && !this.connected) {
        this._backgroundConnect()
        await this._signal.wait()
      ...
      try {
        return await this.rpc.request('add-cores', req, AddCoresEncoding)
      } catch (err) {
        error = err
      }
    }
