...
class BlindPeerClient extends ReadyResource {
  ...
  async addCore (id, opts = {}) {
    ...
    for (let i = 0; i < this.backoffValues.length * 2; i++) {
      let timeout = maxTime - Date.now()
      while (!this.closing && !this.connected && timeout > 0) {
        this._backgroundConnect()
        if (timeout > 0) await this._signal.wait(timeout)
        timeout = maxTime - Date.now()
      }

      if (this.closing) throw new Error('Closing')
      if (timeout <= 0) throw new Error('Connection attempt timed out')
